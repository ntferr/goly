...
import (
	...
	"github.com/ntferr/goly/app/utils"
)
...
func redirect(c *fiber.Ctx) error {
	...
	goly.Clicked += 1
	if err = model.UpdateGoly(goly); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func createGoly(c *fiber.Ctx) error {
	...
	var goly model.Goly
	if err := c.BodyParser(&goly); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
	...
	if err := model.CreateGoly(goly); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func deleteGoly(c *fiber.Ctx) error {
	...
	if err = model.DeleteGoly(id); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"message": "error could not delete goly in db " + err.Error(),
		})
	}
